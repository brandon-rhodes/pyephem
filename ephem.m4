...
')
FUNCTION_BEGIN
FUNCTION_(scanDB, ERRINT db_crack_line (char string[], Obj *op),
...
%name (whichEquatorial) int pref_equatorial;
%name (whichDateFormat) int pref_dateformat;

%{
/* Define a replacement for the pref_get function in libastro's aux.c
   file. */

int pref_equatorial = PREF_TOPO;
int pref_dateformat = PREF_MDY;

int pref_get(Preferences p)
{
	switch (p) {
	case PREF_EQUATORIAL:
		return pref_equatorial;

	case PREF_DATE_FORMAT:
		return pref_dateformat;
	}
	printf ("Internal error - unknown preference: %d\n", p);
	exit(1);
}
%}
